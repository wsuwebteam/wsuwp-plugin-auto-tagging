...
import { FormTokenField, Button, withFilters } from '@wordpress/components';
...
class WSUFlatTermSelector extends Component {
    constructor() {
        super( ...arguments );
    ...
    componentWillUnmount() {
        ...
        invoke( this.allTermsRequest, [ 'abort' ] );
    }
    ...
    autofillTags() {
        ...
        this.getAllTerms().then((allTerms) => {
            const newTerms = allTerms.filter((term) => {
                ...
                return postContent.indexOf(escapeString(term.name.toLowerCase())) !== -1; // very basic matching
            });

            if(newTerms.length > 0){
                this.setState( { availableTerms: newTerms } );
                this.props.onUpdateTerms(
                    this.state.availableTerms.map((t) => t.id),
                    this.props.taxonomy.rest_base
                );
            }
        });
